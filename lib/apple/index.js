...
const responses = {
	...
	21007:
		...
		'Send it to the test environment service instead.',
	21008:
		'This receipt is from the production receipt, ' +
		'but it was sent to the test environment service for verification. ' +
		'Send it to the production environment service instead.',
...
exports.verifyPayment = function (payment, cb) {
	...
	if (payment.excludeOldTransactions !== undefined) {
		assert.equal(
			typeof payment.excludeOldTransactions,
			'boolean',
			'payment.excludeOldTransactions must be a boolean'
		);
